...
class BunkerEnv(MujocoEnv):
    """
    ## Parameters
    - n_lidar (int): Number of LiDAR rays (default: 898)
    ...
    ## Observation Space
    The observation space is a `Box(-1, 1, (n_lidar*3 + 3 + 2), float64)` where the elements are as follows:
    ...
    def __init__(self, xml_path: str, frame_skip: int = 10, render_mode: str | None = None, n_lidar: int = 449, inference_mode: bool = False,
        ...
        # Observation space: all lidar features, relative goal, velocity
        obs_dim = self.n_lidar * 3 + 3 + 2
    ...
    def _get_obs(self) -> np.ndarray:
        """
        We get the lidar points, the relative goal vector, and the velocity.
        """
        ...
        obs = np.concatenate([pts_flat, relative_goal, vel]).astype(np.float64)  # shape (n_lidar_points*3+3+2,)
        return obs
    ...
    def _reward(self, terminated) -> float:
        ...
        w_curr = w_curr_vector[2] # z-component is yaw rate
