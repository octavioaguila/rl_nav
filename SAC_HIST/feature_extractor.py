...
class FeatureExtractor(BaseFeaturesExtractor):
    """
    Hybrid Architecture for SAC HIST (Dense Reward): 
    1. Spatial Stream: PointNet for current LiDAR (N x 3)
    2. Temporal Stream: 1D-CNN for Velocity History (K x 2)
    3. Goal Stream: Relative Goal (3) directly from observation
    ...
    def forward(self, obs):
        """
        obs: Flat tensor with layout [LiDAR (N*3) | Relative Goal (3) | Velocity History (K*2)]
        """
        B = obs.size(0)

        # Split observation
        split_idx_lidar = self.n_lidar * self.n_feat
        split_idx_goal  = split_idx_lidar + 3
        
        lidar_flat   = obs[:, :split_idx_lidar]                 # (B, N*3)
        goal_context = obs[:, split_idx_lidar:split_idx_goal]   # (B, 3)
        hist_flat    = obs[:, split_idx_goal:]                  # (B, K*2)

        # LiDAR Feature Extraction (PointNet)
        pts = lidar_flat.view(B, self.n_lidar, self.n_feat)
        sincos = pts[..., :2]
        d_norm = pts[..., 2]
