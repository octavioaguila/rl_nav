#!/usr/bin/env python3
# -------------------------------------------------------------
#  Run inference with a SAC_HIST policy trained in train_bunker.py
# -------------------------------------------------------------
import os
import sys
import time
...
ckpt = os.path.join(root, "SAC_HIST", "log", run_name, "best_model", "best_model.zip")
...
for ep in range(n_episodes):
    ...
    # We assume the env info or obs contains what we need for metrics
    # If BunkerEnv provides 'initial_dist' in info, use it for SPL
    initial_dist = None
    ...
    for step in range(max_ep_len):
        ...
        # Last element is w_norm
        curr_v, curr_ang_vel = raw_env.get_robot_velocities()
        curr_v = curr_v[0]
        curr_ang_vel = curr_ang_vel[2]

        if initial_dist is None:
            # goal dist is at index n_lidar*3
            # it is the first element after lidar
            dist_norm = obs[0, n_lidar*3]
            # map back to [0, max_dist_diag]
...
# Save Results
results_dir = os.path.join(root, "SAC_HIST", "inference_results")
