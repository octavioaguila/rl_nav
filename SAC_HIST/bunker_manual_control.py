...
XML_PATH = os.path.join(ROOT, "assets", "worlds", "train", "world_1_easy.xml")
...
def main():
    env = BunkerEnv(XML_PATH, render_mode="human")
    obs, info = env.reset()
    ...
    try:
        while not controller.quit:
            ...
            print(f"[INFERENCE] goal_context {goal_context}")
            print(f"[INFERENCE] hist_flat {hist_flat}")
